...
export function PortalSection({ openFeature }: { openFeature: (slug: string) => void }) {
  const handleNav = (e: React.MouseEvent<HTMLAnchorElement>, path: string) => {
    ...
    if (path.startsWith("http")) {
      ...
      const slug = path.split("/features/")[1];
      if (slug) {
        openFeature(slug);
      }
    }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pb-16">
        ...
        <a href="/features/apps" onClick={(e) => handleNav(e, "/features/apps")} className="glass-card p-5 group"><Card iconWrap="rgba(139,92,246,0.15)" icon="fas fa-layer-group text-lg" title="我的应用" body="小工具、小网页、小脚本，把「想做的事」变成「点一下就能做」　 /></a>
        <a href="/features/memory" onClick={(e) => handleNav(e, "/features/memory")} className="glass-card p-5 group"><Card iconWrap="rgba(236,72,153,0.15)" icon="fas fa-hourglass-half text-lg" title="一刻记忀 body="把时间折叠成一帧一帧画面，回头看时，会发现自己真的走了很远　 /></a>
        <a href="/features/wave" onClick={(e) => handleNav(e, "/features/wave")} className="glass-card p-5 group"><Card iconWrap="rgba(34,197,94,0.15)" icon="fas fa-tree text-lg" title="致敬树林" body="HUST 的森林，世界不死，理想不灭。每一阵风吹过，都是一次重新出发　 /></a>
        <a href="/features/share" onClick={(e) => handleNav(e, "/features/share")} className="glass-card p-5 group"><Card iconWrap="rgba(234,179,8,0.15)" icon="fas fa-share-alt text-lg" title="开源共亀 body="写过的代码、做过的表单、踩过的坑，能帮到一个人，就多一份意义　 /></a>
        <a href="/features/revelation" onClick={(e) => handleNav(e, "/features/revelation")} className="glass-card p-5 group"><Card iconWrap="rgba(79,70,229,0.15)" icon="fas fa-lightbulb text-lg" title="人生启示彀 body="一些路过的思考，一些当时没懂、后来突然懂了的小结　 /></a>
