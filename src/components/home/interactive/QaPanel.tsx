    ...
    <div className="glass-card p-6 lg:col-span-2">
      ...
      {!passed ? (
        ...
        <div id="ask-step">
          ...
          <textarea id="qa-input" value={qaInput} onChange={(e) => setQaInput(e.target.value)} className="w-full bg-black/30 border border-white/15 rounded-xl p-3 text-sm focus:outline-none focus:border-[color:var(--accent-main)] transition-colors" rows={3} placeholder="写下你的问题或想说的�?.." />
          <div className="flex justify-between items-center mt-3">
            <span className="text-[11px] text-gray-500">数据仅保存在本地存储</span>
            <button type="button" onClick={submitQuestion} className="px-5 py-2 rounded-full font-bold text-sm transition-all hover:scale-105" style={{ background: "var(--accent-main)", color: "#0a0f1a" }}>发�?<i className="fas fa-paper-plane ml-1" /></button>
          </div>
